...
class Utilitas extends CI_Controller {
    ...
    function tambah_user_bpp(){
        $data['page_title']= 'INPUT S P P U P';
        $this->template->set('title', 'INPUT S P P U P');   
        $this->template->load('template','master/tambah_user_bpp',$data) ; 
    }

    function simpan_user(){
    	$username=$this->input->post('user');
    	$password=md5($this->input->post('pswd'));
    	$namaskpd=$this->input->post('nama');
    	$kode    =$this->session->userdata('kdskpd');
    	$namau    =$this->db->query("select top 1 nm_skpd from ms_skpd where kd_skpd='$kode'")->row()->nm_skpd;

    	$kodeskpd=$this->db->query("SELECT count(*)+1 total from [user] where left(kd_skpd,17)=left('$kode',17) and type='2' and jenis='2' and bidang='5'")->row()->total;
    	$kodeskpd2= $kodeskpd < 10 ? '0'.$kodeskpd : $kodeskpd;
    	$iduser=$this->db->query("SELECT max(cast(id_user as int))+1 id_user from [user] where cast(id_user as int) between 202 and 400")->row()->id_user;
    	$insert_msskpd=$this->db->query("INSERT into ms_skpd (kd_skpd, statu, status_sempurna, status_ubah) select top 1 left('$kode',17)+'.00$kodeskpd2' kd_skpd,statu, status_sempurna, status_ubah from trhrka where kd_skpd='$kode'");
    	$insert=$this->db->query("INSERT into [user] (id_user,user_name, password, type, nama, kd_skpd, jenis, status, bidang) values ('$iduser','$username','$password','2','$namau ('+'$namaskpd)',left('$kode',17)+'.00$kodeskpd2','2','0','5') ");
    	$insert_otori=$this->db->query("INSERT into otori select '$iduser' id_user, menu_id, akses from otori where user_id='200'");
    	$insert_trhrka=$this->db->query("INSERT into trhrka (kd_skpd, statu, status_sempurna, status_ubah) select top 1 left('$kode',17)+'.00$kodeskpd2' kd_skpd,statu, status_sempurna, status_ubah from trhrka where kd_skpd='$kode'");
    	$insert_sclient=$this->db->query("INSERT into sclient (kd_skpd, provinsi, kab_kota, daerah, thn_ang) select top 1 left('$kode',17)+'.00$kodeskpd2' kd_skpd, provinsi, kab_kota, daerah, thn_ang from sclient where kd_skpd='$kode'");
