...
class setor extends CI_Controller {
	...
	function load_rek_pot(){
        ...
        $query1 = $this->db->query("SELECT a.kd_rek6, a.nm_rek6 FROM trdtransout a
			INNER JOIN trhtransout b ON a.no_bukti = b.no_bukti AND a.kd_skpd = b.kd_skpd WHERE
			 a.no_sp2d = '$sp2d' AND a.kd_kegiatan = '$kd_giat_pot'");  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
        { 
    ...
    function pot_in() {
        ...
        $sql = "SELECT *, 'Klik 2x Edit NTPN' oke FROM trdtrmpot where no_bukti='$bukti' AND kd_skpd='$kd_skpd'";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
     ...
     function simpan_strpot()
	{
		...
		$kd_rek0= $this->input->post('rek0');                         
		$nm_rek0= $this->input->post('nmrek');                 
		$rekanan= $this->input->post('rekanan');                         
		$dir= $this->input->post('dir');                         
		$alamat= $this->input->post('alamat');                         
		$usernm= $this->session->userdata('pcNama');
		$nnt= $this->input->post('nnt');      
		$nokas_str= $this->input->post('nokas_str');      
		...
		$last_update=  date('Y-m-d H:i:s');
           ...
           $query2 ="INSERT into trhstrpot(no_bukti,tgl_bukti,ket,username,tgl_update,kd_skpd,nm_skpd,no_terima,npwp,jns_spp,nilai,no_sp2d,kd_sub_kegiatan,nm_sub_kegiatan,kd_rek6,nm_rek6,nmrekan,pimpinan,alamat,no_nnt,npwpd,no_kas_str) 
                    values('$no_bukti','$tgl_bukti','$ket','$usernm','$last_update','$kd_skpd','$nm_skpd','$no_terima','$npwp','$jns_spp','$nilai','$no_sp2d','$kd_giat','$nm_giat','$kd_rek0','$nm_rek0','$rekanan','$dir','$alamat','$nnt','$npwpd','$nokas_str')";
			...
			$query3 ="INSERT INTO trdstrpot (no_bukti,kd_rek6,nm_rek6,nilai,kd_skpd,kd_rek_trans) 
						SELECT $no_bukti,kd_rek6,nm_rek6,nilai,kd_skpd,kd_rek_trans FROM trdtrmpot WHERE no_bukti = '$no_terima' AND kd_skpd='$kd_skpd'";
                    $asg3 = $this->db->query($query3);	
...
function simpan_strpot_edit()
	{
		...
		$last_update=  date('Y-m-d H:i:s');
           ...
           $asg = $this->db->query($sql);       
           $query2 ="insert into trhstrpot(no_bukti,tgl_bukti,ket,username,tgl_update,kd_skpd,nm_skpd,no_terima,npwp,jns_spp,nilai,no_sp2d,kd_sub_kegiatan,nm_sub_kegiatan,kd_rek6,nm_rek6,nmrekan,pimpinan,alamat,no_nnt,npwpd,no_kas_str) 
                    values('$no_bukti','$tgl_bukti','$ket','$usernm','','$kd_skpd','$nm_skpd','$no_terima','$npwp','$jns_spp','$nilai','$no_sp2d','$kd_giat','$nm_giat','$kd_rek0','$nm_rek0','$rekanan','$dir','$alamat','$nnt','$npwpd','$nokas_str') ";
                    $asg2 = $this->db->query($query2);    
    ...
    function load_pot_out() {
       $kd_skpd     = $this->session->userdata('kdskpd');
        ...
        $sql = "SELECT top $rows kd_rek6, nm_rek6, no_bukti,tgl_bukti,no_terima,kd_skpd,no_sp2d,RTRIM(jns_spp) as jns_spp,
				nm_skpd,nm_sub_kegiatan,kd_sub_kegiatan,nmrekan,pimpinan, alamat,npwp,ket,nilai,no_nnt,npwpd,no_kas_str
        ...
        { 
            ...
            $row[] = array(
                        ...
                        'nilai' => $resulte['nilai'],
                        'npwp' => $resulte['npwp'],
						'no_nnt' => $resulte['no_nnt'],
						'npwpd' => $resulte['npwpd'],
						'no_kas_str' => $resulte['no_kas_str']
